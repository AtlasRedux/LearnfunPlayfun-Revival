...
static void SaveFuturesHTML(const vector<Future> &futures,
  ...
  // Set to true before constructing PlayFun to resume from a
  // previously saved progress file instead of starting fresh.
  static bool resume_mode = false;

  // Global pointer for the Ctrl+C handler to save progress on exit.
  struct PlayFun;
  static PlayFun *g_playfun = nullptr;
...
struct PlayFun {
  PlayFun() : watermark(0), log(NULL), rc("playfun") {
    ...
    // Determine the warmup length from the original movie.
    // Warmup = all frames up to and including the first keypress.
    size_t warmup_length = 0;
    {
      bool saw_input = false;
      for (size_t i = 0; i < solution.size(); i++) {
        warmup_length++;
        saw_input = saw_input || solution[i] != 0;
        if (i > FASTFORWARD && saw_input) break;
      }
    }

    if (resume_mode) {
      // Resume from a previously saved progress file.
      string progress_file = game + "-playfun-futures-progress.fm2";
      vector<string> saved_subtitles;
      vector<uint8> saved = SimpleFM2::ReadInputsAndSubtitles(
          progress_file, &saved_subtitles);
      if (saved.empty()) {
        fprintf(stderr, "Resume failed: could not read %s\n",
                progress_file.c_str());
        abort();
      }

      fprintf(stderr,
              ANSI_GREEN "Resuming" ANSI_RESET " from %s (%d inputs).\n",
              progress_file.c_str(), (int)saved.size());

      // Replay all saved inputs to rebuild emulator state and checkpoints.
      // Use recovered subtitles so annotations are preserved.
      for (size_t i = 0; i < saved.size(); i++) {
        const string &sub = saved_subtitles[i].empty()
            ? (i < warmup_length ? "warmup" : "resume")
            : saved_subtitles[i];
        Commit(saved[i], sub);
        if (i < warmup_length) watermark++;

        // Progress indicator every 1000 frames.
        if ((i + 1) % 1000 == 0 || i + 1 == saved.size()) {
          fprintf(stderr, "\r  Replaying: %d / %d frames...",
                  (int)(i + 1), (int)saved.size());
        }
      }
      fprintf(stderr, "\n");

      CHECK(saved.size() > 0 && "Progress file was empty.");
      fprintf(stderr,
              ANSI_GREEN "Resume complete." ANSI_RESET
              " %d inputs, %d checkpoints, watermark at %d.\n",
              (int)movie.size(), (int)checkpoints.size(), watermark);
    } else {
      // Normal startup: replay warmup frames from the original movie.
      size_t start = 0;
      bool saw_input = false;
  ...
  void Helper(int port) {
    ...
    for (;;) {
      ...
      connections++;
      string connline = StringPrintf("[%d] Connection #%d from %s",
         port,
         connections,
         server.PeerString().c_str());
      term.Output(connline + "\n");

      // Process one request per connection (matching original SDL_net
      // behavior).  Persistent connections caused crashes due to
      // accumulated emulator state corruption.
      {
        HelperRequest hreq;
        if (!server.ReadProto(&hreq)) {
          // Connection dropped; go back to accept.
          server.Hangup();
          continue;
        }

        requests_served++;
        string line = StringPrintf("[%d] req #%d", port, requests_served);
      ...
      // Hangup after each request (matching original SDL_net behavior).
      server.Hangup();
    }
  }
  ...
  void ParallelStep(const vector< vector<uint8> > &nexts,
    ...
    *best_next_idx = 0;
    ...
    typedef GetAnswers<HelperRequest, PlayFunResponse> GA;
    // Use persistent pool when available (avoids per-request connect overhead).
    GA *ga = pool_
      ? new GA(pool_, requests)
      : new GA(ports_, requests);
    ga->Loop();

    const vector<GA::Work> &work = ga->GetWork();
  ...
  void Master(const vector<int> &helpers) {
    ...
    // Use legacy one-connection-per-request mode (matching original
    // SDL_net behavior).  The helper now processes one request per
    // connection and hangs up, so persistent pooling would just
    // cause stale-socket retries.
    // pool_ is left as nullptr; GetAnswers uses the legacy constructor.
    fprintf(stderr, "[MASTER] Using one-connection-per-request mode "
            "for %d helpers.\n", (int)helpers.size());
  ...
  void TryImprove(Checkpoint *start,
    ...
    typedef GetAnswers<HelperRequest, TryImproveResponse> GA2;
    GA2 *ga2 = pool_
      ? new GA2(pool_, requests)
      : new GA2(ports_, requests);
    ga2->Loop();

    const vector<GA2::Work> &work = ga2->GetWork();
  ...
  vector<int> ports_;
  // Persistent connection pool (nullptr if not using pool mode).
  HelperPool *pool_ = nullptr;
...
static LONG WINAPI CrashHandler(EXCEPTION_POINTERS *ep) {
  ...
  // Walk the stack with StackWalk64.
  ...
  for (int i = 0; i < 30; i++) {
    if (!StackWalk64(machType, proc, thread, &sf, ctx,
    ...
    if (SymFromAddr(proc, sf.AddrPC.Offset, &disp, sym)) {
      fprintf(stderr, "  [%2d] %s + 0x%llx  (0x%p)\n",
...
#ifdef _WIN32
// Console Ctrl handler — saves progress before exiting on Ctrl+C.
static BOOL WINAPI CtrlHandler(DWORD ctrlType) {
  if (ctrlType == CTRL_C_EVENT || ctrlType == CTRL_CLOSE_EVENT ||
      ctrlType == CTRL_BREAK_EVENT) {
    if (g_playfun) {
      fprintf(stderr, "\n" ANSI_YELLOW "Caught signal — saving progress..."
              ANSI_RESET "\n");
      g_playfun->SaveMovie();
      fprintf(stderr, ANSI_GREEN "Progress saved." ANSI_RESET "\n");
    }
...
int main(int argc, char *argv[]) {
  // Make stderr unbuffered so CHECK/abort messages are visible.
  ...
  // Enable ANSI escape sequence processing so the colored progress
  // bars render correctly in modern Windows consoles without ansicon.
  {
  ...
  // Check for --resume flag anywhere in the arguments.
  for (int i = 1; i < argc; i++) {
    if (0 == strcmp(argv[i], "--resume")) {
      resume_mode = true;
      break;
    }
  }

  // Warn about existing progress file on fresh runs (non-fatal).
  if (!resume_mode) {
    map<string, string> cfg = Util::ReadFileToMap("config.txt");
    string g = cfg["game"];
    if (!g.empty()) {
      string progress_file = g + "-playfun-futures-progress.fm2";
      FILE *check = fopen(progress_file.c_str(), "r");
      if (check) {
        fclose(check);
        fprintf(stderr,
                ANSI_YELLOW "WARNING:" ANSI_RESET
                " Existing progress file found: %s\n"
                "It will be overwritten when new progress is saved.\n"
                "Use " ANSI_GREEN "--resume" ANSI_RESET
                " to continue from where you left off instead.\n",
                progress_file.c_str());
      }
    }
  }
  ...
  if (argc >= 2) {
    if (0 == strcmp(argv[1], "--helper")) {
      ...
      for (int i = 2; i < argc; i++) {
        if (0 == strcmp(argv[i], "--resume")) continue;
        int hp = atoi(argv[i]);
        if (!hp) {
          fprintf(stderr,
      ...
      // Auto mode: detect CPU cores, spawn helpers, then run master.
      SYSTEM_INFO si;
      GetSystemInfo(&si);
      int numHelpers = (int)si.dwNumberOfProcessors - 1;
      if (numHelpers < 1) numHelpers = 1;
      // Look for an explicit helper count (skip --resume flag).
      for (int i = 2; i < argc; i++) {
        if (0 != strcmp(argv[i], "--resume")) {
          int n = atoi(argv[i]);
          if (n >= 1) numHelpers = n;
      ...
      fprintf(stderr, "Auto mode: spawning %d helper(s)...\n", numHelpers);

      // Job object: auto-kill all helpers when master exits.
      ...
      for (int i = 0; i < numHelpers; i++) {
        ...
        if (CreateProcessA(NULL, cmdLine, NULL, NULL, FALSE,
          ...
          fprintf(stderr, "  Spawned helper on port %d (PID %d)\n",
                  port, (int)pi.dwProcessId);
        } else {
          fprintf(stderr, "  Failed to spawn helper on port %d: %d\n",
                  port, (int)GetLastError());
        }
      }

      // Give helpers time to bind their ports.
      fprintf(stderr, "Waiting for helpers to initialize...\n");
      Sleep(3000);

      // Bring our console back to the foreground (helper windows buried it).
      {
      ...
      // Cleanup: closing the job handle kills all helper processes.
