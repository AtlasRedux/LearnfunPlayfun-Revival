...
struct StateCache {
  ...
  struct HashFunction {
    size_t operator ()(const Key &k) const {
      CHECK(k.second);
      if (k.second->empty()) {
        // Safety net: hash empty vectors by input byte only.
        return (size_t)k.first;
      }
      return CityHash64WithSeed((const char *)&((*k.second)[0]),
				k.second->size(),
				k.first);
    }
  };
  ...
  void Resize(uint64 ll, uint64 ss) {
    ...
    // Recover memory.
    // IMPORTANT: We must erase from the hash table BEFORE deleting
    // the key/value vectors.  MSVC's unordered_map::erase(iterator)
    // recomputes the hash of the key to locate the correct bucket,
    // so the key's vector must still be alive at that point.
    // (libstdc++ caches hash values in nodes, hiding this bug on Linux.)
    for (Hash::iterator it = hashtable.begin();
	 it != hashtable.end(); /* in loop */) {
      Hash::iterator next(it);
      ++next;
      const vector<uint8> *key_vec = it->first.second;
      vector<uint8> *val_vec = it->second.second;
      hashtable.erase(it);   // erase first (needs live key for hashing)
      delete key_vec;         // then free memory
      delete val_vec;
      it = next;
    }
  ...
  void MaybeResize() {
	...
	if (it->second.first < minseq) {
	  Hash::iterator next(it);
	  ++next;
	  // Save pointers, erase (needs live key for hashing), then free.
	  const vector<uint8> *key_vec = it->first.second;
	  vector<uint8> *val_vec = it->second.second;
	  hashtable.erase(it);
	  delete key_vec;
...
void Emulator::SaveEx(vector<uint8> *state, const vector<uint8> *basis) {
  ...
  // Compress with LZ4 (much faster decompression than zlib).
...
void Emulator::LoadEx(vector<uint8> *state, const vector<uint8> *basis) {
  // Decompress LZ4. Header: 4 bytes uncompressed len + 4 bytes compressed len.
