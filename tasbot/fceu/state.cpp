...
bool FCEUSS_SaveMS(EMUFILE* outstream, int compressionLevel, std::vector<uint8> *basis)
{
  ...
  if(compressionLevel != Z_NO_COMPRESSION &&
    ...
    if ((int)compressed_buf.size() < maxcomprlen) compressed_buf.resize(maxcomprlen);
    cbuf = &compressed_buf[0];
    // do compression with LZ4
    comprlen = LZ4_compress_default((const char*)memory_savestate.buf(),
                                    (char*)cbuf, len, maxcomprlen);
    if (comprlen <= 0) ok = false;
  }

  // Simplified header; in-ram only.
  uint8 header[8] = "";
  FCEU_en32lsb(header, totalsize);
  FCEU_en32lsb(header + 4, (uint32)comprlen);
...
bool FCEUSS_LoadFP(EMUFILE* is, ENUM_SSLOADPARAMS params, std::vector<uint8> *basis) {
  ...
  if (comprlen != -1) {
    // the savestate is compressed: read from is to compressed_buf, then decompress with LZ4
    if ((int)compressed_buf.size() < comprlen) compressed_buf.resize(comprlen);
    is->fread(&compressed_buf[0], comprlen);

    int result = LZ4_decompress_safe((const char*)&compressed_buf[0],
...
void AddExState(void *v, uint32 s, int type, const char *desc) {
  ...
  if(desc) {
    char *tmp = (char *)FCEU_malloc(strlen(desc)+1);
    strcpy(tmp,desc);
    SFMDATA[SFEXINDEX].desc=tmp;
  } else {
