...
class TASBotGUI:
    ...
    def _build_ui(self):
        ...
        self.fceux_exe = self._find_fceux()
        self.btn_replay = ttk.Button(
            btn_frame, text="🎬  Watch Replay  (FCEUX)",
            command=self._watch_replay,
            state="normal" if self.fceux_exe else "disabled")
    ...
    def _check_executables(self):
        ...
        if missing:
            self._log(
                ...
                "err")
        if not self.fceux_exe:
            self._log("NOTE: FCEUX not found in FCEUX\\ subfolder — "
                      "replay button disabled.\n")

    def _find_fceux(self) -> Path | None:
        """Return the path to fceux64.exe or fceux.exe, or None."""
    ...
    def _watch_replay(self):
        """Launch FCEUX to play back the progress FM2."""
        rom = self.rom_var.get().strip()
        if not rom:
            messagebox.showwarning("Missing ROM", "Please select a ROM file.")
            return
        game = Path(rom).stem
        progress = APP_DIR / f"{game}-playfun-futures-progress.fm2"
        if not progress.exists():
            messagebox.showwarning(
                "No progress file",
                f"No progress file found:\n{progress.name}\n\n"
                "Run playfun first to generate one.")
            return
        cmd = [str(self.fceux_exe),
               "-playmovie", str(progress), str(APP_DIR / rom)]
        self._log(f"$ {' '.join(cmd)}\n", "info")
        try:
            subprocess.Popen(cmd, cwd=str(FCEUX_DIR))
        except Exception as exc:
            messagebox.showerror("FCEUX launch failed", str(exc))
